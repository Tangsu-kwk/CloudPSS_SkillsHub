...
当前实现保留模型原有 `diagram-edge`。新增故障时通过显式 `diagram-edge` 连接目标端口与故障端口、故障接地端口与 GND 端口；组件 `pins` 字段只作为兼容信息，不替换可视拓扑边。已有母线支路边必须原样保留。
...
新增 `faultresistor_3p` 时，先解析目标端口所在的逻辑网络 Pin 名。已有输电支路不构成“端口占用”限制；同一网络的多个元件和故障通过相同 Pin 名并联。

请求中的目标可以使用组件 ID，也可以使用唯一的显示名称。创建预览和实际执行时都必须先把它解析为当前模型中真实存在的组件 ID；`diagram-edge.source.cell` 和 `diagram-edge.target.cell` 只能写真实组件 ID，不得写 `args.Name`、标签或其他显示名称。显示名称不存在或命中多个组件时停止创建并要求提供组件 ID。

目标引脚必须同时满足：

1. 目标组件、引脚名称和引脚方向可以从正式模型对象确认；
2. 对普通设备，没有任何正式 `diagram-edge` 连接到该引脚；对 `*_newBus_3p` 等明确的三相母线/汇流节点，已有连接不视为占用，可在同一端口并联；
3. 没有现有故障、接地支路或其他设备占用该引脚；
4. 连接该引脚不会要求 Skill 自动拆分线路、重连已有设备或创建未知中间拓扑。

若目标不是明确的母线/汇流节点且无法确认引脚是否空闲，停止新增并说明缺少的拓扑信息。不得把普通设备的已用引脚当作可并联端口。
...
必须使用参考组件定义创建故障和 GND，并为故障到目标、故障到 GND 分别创建正式 `diagram-edge`。故障 `pins['0']` 可记录目标网络 Pin 名，`pins['1']` 为 `GND`；GND 组件使用 `pins['0'] = 'GND'`。不得删除目标端口已有的 `diagram-edge`。
